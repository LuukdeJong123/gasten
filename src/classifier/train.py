...
def train(C, opt, crit, train_loader, val_loader, test_loader, acc_fun, args, name, model_params, device, out_dir):
    stats = {
        'best_loss': float('inf'),
        'best_epoch': 0,
...
def main():
    ...
    stats, cp_path = \
        train(C, opt, criterion, train_loader, val_loader,
              test_loader, acc_fun, args, name, model_params, device, out_dir)
