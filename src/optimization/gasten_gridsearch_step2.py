...
def main():
    ...
    with open(f'{os.environ["FILESDIR"]}/step-1-best-grid-search-config-{pos_class}v{neg_class}.txt', 'r') as file:
        gan_path = file.read()
    ...
    for params in tqdm(list(ParameterSampler(param_grid, n_iter=10000, random_state=rng))):
        iteration += 1
